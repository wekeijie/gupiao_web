  ...
  <div class="bg-color ui-min-height">
    ...
    <div class="px-3 mt-n16">
      ...
      <v-sheet class="bg-white rounded-lg py-3 mt-3 px-3">
        ...
        <v-row class="ma-0 pa-0">
          <v-select
            v-model="applyAmountId"
            label="Select"
            :items="store.state.contract.limitList"
            variant="underlined"
            :item-props="cuelAmount"
            item-value="id"
            @update:modelValue="selelctLimitApply"
          >
          </v-select>
        </v-row>
      </v-sheet>
...
const cuelAmount = (item) => {
  return {
    title: "¥ " + item.limit * amount.value,
    subtitle: item.title,
  };
};
...
const getLimit = (type) => {
  store.dispatch("contract/getLimit", type).then((d) => {
    selelctLimitApply(d[0].id);
  });
};

const selelctLimitApply = (id) => {
  const data = store.state.contract.limitList.find((item) => item.id == id);

  applyAmountId.value = data.id;
