<template>
  <div>
    ...
    <div class="input-number">
      ...
      <v-text-field model-value="500.00" variant="underlined" hide-details="auto" required prefix="￥"
        size="25"></v-text-field>
    ...
    <div class="flexStart limit-list-box">
      <div class="limit-list " :class="selectIndex == item ? 'activeBg' : ''" v-for="item in 11" :key="item"
    ...
    <div class="apply-btn" @click="isApply = true">立即申请</div>

    <v-bottom-sheet v-model="isProp">
      <v-list>
        <div class="flexBetween sheet-top">
          <div class="sheet-sure">选择合约类型</div>
          <img src="../../assets/img/close.png" alt="" @click="isProp = false">
        </div>
        <v-list-item class="flexCenter" v-for="tile, index in tiles" :key="tile.title" @click="selectType(index)">
          <div class="flexBetween type-list">

            <v-list-item-title>{{ tile.title }}</v-list-item-title>
            <img src="../../assets/img/select.png" alt="" v-if="index == typeIndex">
          </div>
        </v-list-item>
      </v-list>
    </v-bottom-sheet>
    ...
    <v-dialog v-model="isApply" persistent class="flexCenter">
      ...
      <div class="prop-tips">
        ...
        <div class="prop-btn flexCenter">
          ...
          <v-btn @click="isApply = false" color="#fb5c39">确认</v-btn>
        </div>
      </div>
    </v-dialog>
  </div>
