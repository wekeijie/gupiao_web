  ...
  <div>
    ...
    <div class="beginner-box">
      <div class="pcontent-title">
        明星投资者：{{ store.state.copyTrade.rookie.name }}
      </div>
      <div class="pcontent-info">
        分成比例：{{ store.state.copyTrade.rookie.commission + "%" }}
      </div>
      ...
      <v-btn block color="#fa3534" @click="sub" class="mt-5">确认跟单</v-btn>
      <div class="interests">
        <h3>跟单说明</h3>
...
const sub = () => {
  if (searchCont.value == 100 || searchCont.value % 100 === 0) {
    store
      .dispatch("copyTrade/apply", {
        amount: searchCont.value,
