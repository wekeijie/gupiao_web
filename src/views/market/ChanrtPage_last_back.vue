  ...
  <div class="chanrt-box">
    <page-header>
      <template v-slot:headerCenter>
        ...
        <p v-show="stockNewInfo.f292 != 2">
          <span>{{ extractTxtValue(stockNewInfo.f292) }}</span>
        </p>
      </template>
    ...
    <div
      ...
      :class="'text-' + watchStringToColor(stockNewInfo.f170)"
    >
      <h2>{{ marketDataFormat(stockNewInfo.f43, stockNewInfo.f59) }}</h2>
      <div>
        <p>{{ marketDataFormat(stockNewInfo.f169, stockNewInfo.f59) }}</p>
        <p>{{ marketDataFormat(stockNewInfo.f170, stockNewInfo.f59) + "%" }}</p>
      </div>
    </div>
    ...
    <div class="flexStart stati-box">
      <div class="stati-list">
        <p>最高</p>
        <span>{{ marketDataFormat(stockNewInfo.f44, stockNewInfo.f59) }}</span>
      </div>
      <div class="stati-list">
        <p>最低</p>
        <span>{{ marketDataFormat(stockNewInfo.f45, stockNewInfo.f59) }}</span>
      </div>
      <div class="stati-list">
        <p>涨停</p>
        <span>{{ marketDataFormat(stockNewInfo.f51, stockNewInfo.f59) }}</span>
      </div>
      <div class="stati-list">
        <p>跌停</p>
        <span>{{ marketDataFormat(stockNewInfo.f52, stockNewInfo.f59) }}</span>
      </div>
      <div class="stati-list">
        <p>今开</p>
        <span>{{ marketDataFormat(stockNewInfo.f46, stockNewInfo.f59) }}</span>
      </div>
      <div class="stati-list">
        <p>昨收</p>
        <span>{{ marketDataFormat(stockNewInfo.f60, stockNewInfo.f59) }}</span>
      </div>
      <div class="stati-list">
        <p>振幅</p>
        <span>{{
          marketDataFormat(stockNewInfo.f170, stockNewInfo.f59) + "%"
        }}</span>
      </div>
      <div class="stati-list">
        <p>换手率</p>
        <span>{{
          marketDataFormat(stockNewInfo.f168, stockNewInfo.f59) + "%"
        }}</span>
      </div>
      <div class="stati-list">
        <p>成交量</p>
        <span>{{ toWanString(stockNewInfo.f47) + "手" }}</span>
      </div>
      <div class="stati-list">
        <p>成交额</p>
        <span>{{ toYiString(stockNewInfo.f48) }}</span>
      </div>
      <div class="stati-list">
        <p>市盈率</p>
        <span>{{
          marketDataFormat(stockNewInfo.f162, stockNewInfo.f59) + "%"
        }}</span>
      </div>
      <div class="stati-list">
        <p>市净率</p>
        <span>{{
          marketDataFormat(stockNewInfo.f167, stockNewInfo.f59) + "%"
        }}</span>
      </div>
    </div>
...
const getNewStockInfo = async () => {
  const rsp = await store.dispatch(
    "market/getStockNewInfo",
    prefix_symbol.value
  );
  stockNewInfo.value = rsp;
};
