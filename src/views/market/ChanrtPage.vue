  ...
  <div class="chanrt-box">
    ...
    <page-header>
      <template v-slot:headerCenter>
        ...
        <p v-show="is_open">休市中 <span>{{ last_update_date }}</span></p>
      </template>
      <template v-slot:headerRight>
        <img class="refresh" src="../../assets/img/refresh.png" alt="" />
      </template>
    </page-header>
    <div class="flexCenter argument-box" :class="'text-' + watchStringToColor(store.state.market.stock_info.l)">
    ...
    <div class="flexStart stati-box">
      ...
      <div class="stati-list">
        <p>成交量</p>
        <span>{{ toWanString(store.state.market.stock_info.jiaoliang) + '手' }}</span>
      </div>
      <div class="stati-list">
        <p>成交额</p>
    ...
    <div class="flexBetween trading-box">
      <div @click="pushMeQuote">
        <img src="../../assets/img/selef.png" alt="" v-if="store.state.watchlist.status">
        <img src="../../assets/img/selefNo.png" alt="" v-else>
        <p>自选</p>
      </div>
      <div class="trading-btn" @click="goTrading('/Trading',title,Symbol,prefix_symbol)">交易</div>
...
const RequestHistoryMinuteData = (data,callback,option) => {
  request.post('market/fiveStock',{'symbol':prefix_symbol.value}).then(d => {
    let klineData = jsonToFiveHQChartMinuteData(d,d['prePrice'])
    let hqChartData = {"symbol": d.code + '.sh',"name": d.name,"data": klineData,"code": 0}
    callback(hqChartData)
  })
}
...
const pushMeQuote = () => {
  ...
  store.dispatch('watchlist/push',{'code':parseFloat(prefix_symbol.value),'name':title.value,'price':store.state.market.stock_info.price,'percentage':store.state.market.stock_info.e}).then(d => {
...
const checkDateStatus = () =>{
  let beijingDate = new Date().toLocaleDateString("zh-CN", {
      timeZone: "Asia/Shanghai", 
      year: 'numeric', 
      month: '2-digit', 
      day: '2-digit'
  }).replace(/\//g, '-');
  let beijingTime = new Date().toLocaleString("zh-CN", {timeZone: "Asia/Shanghai", hour12: false, timeStyle: 'short'});
...
const checkTime = () => {
  let currentBeijingTime = new Date(new Date().toLocaleString("zh-CN", {
      timeZone: "Asia/Shanghai",
      hour12: false
  }));
  ...
  if(currentBeijingTime > afternoonStart && currentBeijingTime < afternoonEnd){
    return true
  }
  return false
}
